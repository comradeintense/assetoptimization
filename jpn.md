...
## Intro
はじめまして。私は「ComradeIntense」です。Cities Skylinesのアセットパフォーマンスをより最適化するためのヒントやトリックを紹介します。

この先、私が自分で学んだこと、または他の人（主に私の師匠であるRonyx69）から教わったこと、あるいは仕事で学んだことを、他の人にも伝えていきたいと考えています。もしこのガイドに同意できないことがあれば、遠慮なくご連絡いただき、訂正してください。

この小さなガイドは、あなたがすでにアセットの作り方を知っていることを前提としています（良い情報源はRonyxのウェブサイト：http://cslmodding.info ）。しかし、あなたがここにいるのは、自分の決定が他の人のゲームのパフォーマンスにどのように影響するか、そしてそれに対して何ができるかを知りたいと思うからでしょう。

できるだけ短く、要点を絞って書きますが、あなたにとって重要な、オタク的な部分があちこちに出てきますので、じっくりと、読み飛ばさないようにしてください。
...
簡単に言うと、ドローコールとは、CPUがGPUに指示する命令のことです。メッシュをロードすると、CPU はメッシュの各頂点のワールド空間での位置とマテリアル（複数）を計算し、その後 GPU に指示します。
```"こんにちは、GPU です。ワールドの (x,y,z) の位置には頂点があり、こことこことここにも頂点があり、そのすべての頂点がこのメッシュ全体を構成しています。これらの頂点の間に、この位置にこのマテリアルを UV マップや法線マップなどに適用して、画面に描画する必要があるのです!”```  
